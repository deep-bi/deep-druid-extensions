## Druid Filtering Ext
*This repository contains different filtering extensions*
...
```
$ tar xzf target/druid-filtering-ext-31.0.0-bin.tar.gz
$ ls druid-filtering-ext/
LICENSE                  README.md               druid-filtering-ext-31.0.0.jar
```
...
1. Copy `druid-filtering-ext` into your Druid `extensions` directory.
2. Edit `conf/_common/common.runtime.properties` to add `"druid-filtering-ext"` to `druid.extensions.loadList`. (
   Edit `conf-quickstart/_common/common.runtime.properties` too if you are using the quickstart config.)
   It should look like: `druid.extensions.loadList=["druid-filtering-ext"]`. There may be a few other extensions
   there too.
3. Restart Druid.