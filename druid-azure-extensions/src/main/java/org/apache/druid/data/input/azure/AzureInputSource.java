...
public class AzureInputSource extends CloudObjectInputSource {
    ...
    public AzureInputSource(
        ...
        this.azureCloudBlobIterableFactory =
                Preconditions.checkNotNull(azureCloudBlobIterableFactory, "AzureCloudBlobIterableFactory");
        ...
        class SplitWidget implements CloudObjectSplitWidget {
            ...
            public Iterator<LocationWithSize> getDescriptorIteratorForPrefixes(List<URI> prefixes) {
                return Iterators.transform(
                        azureCloudBlobIterableFactory
                                .create(getPrefixes(), inputDataConfig.getMaxListingLength())
                                .iterator(),
                        blob -> {
                            try {
                                return new LocationWithSize(
                                        blob.getContainerName(), blob.getName(), blob.getBlobLength());
            ...
            public long getObjectSize(CloudObjectLocation location) {
                try {
                    final CloudBlob blobWithAttributes =
                            storage.getBlobReferenceWithAttributes(location.getBucket(), location.getPath());
    ...
    public String toString() {
        return "AzureInputSource{" + "uris="
                + getUris() + ", prefixes="
                + getPrefixes() + ", objects="
                + getObjects() + ", objectGlob="
                + getObjectGlob() + '}';
    }
}
