...
public class AzureUtils {
    ...
    public static void deleteObjectsInPath(
            ...
            throws Exception {
        AzureCloudBlobIterable azureCloudBlobIterable = azureCloudBlobIterableFactory.create(
                ImmutableList.of(new CloudObjectLocation(bucket, prefix).toUri("azure")), config.getMaxListingLength());
        ...
        while (iterator.hasNext()) {
            ...
            if (filter.apply(nextObject)) {
                deleteBucketKeys(
                        storage, accountConfig.getMaxTries(), nextObject.getContainerName(), nextObject.getName());
            }
        }
    }

    private static void deleteBucketKeys(AzureStorage storage, int maxTries, String bucket, String prefix)
            throws Exception {
