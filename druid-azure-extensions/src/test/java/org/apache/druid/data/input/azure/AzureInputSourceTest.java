...
public class AzureInputSourceTest extends EasyMockSupport {
    ...
    private static final InputFormat INPUT_FORMAT =
            new JsonInputFormat(new JSONPathSpec(true, null), null, false, null, null);
    ...
    public void test_createEntity_returnsExpectedEntity() {
        ...
        EasyMock.expect(inputSplit.get())
                .andReturn(ImmutableList.of(CLOUD_OBJECT_LOCATION_1))
                .times(2);
        replayAll();
        ...
        AzureEntity actualAzureEntity =
                azureInputSource.createEntity(inputSplit.get().get(0));
    ...
    public void test_createSplits_successfullyCreatesCloudLocation_returnsExpectedLocations() throws Exception {
        ...
        EasyMock.expect(azureCloudBlobIterableFactory.create(prefixes, MAX_LISTING_LENGTH))
        ...
        Stream<InputSplit<List<CloudObjectLocation>>> cloudObjectStream =
                azureInputSource.createSplits(INPUT_FORMAT, new MaxSizeSplitHintSpec(null, 1));

        List<List<CloudObjectLocation>> actualCloudLocationList =
                cloudObjectStream.map(InputSplit::get).collect(Collectors.toList());
        verifyAll();
    ...
    public void test_getPrefixesSplitStream_withObjectGlob_successfullyCreatesCloudLocation_returnsExpectedLocations()
        ...
        expectedCloudBlobsIterator =
                Iterators.filter(expectedCloudBlobsIterator, object -> m.matches(Paths.get(object.getName())));
