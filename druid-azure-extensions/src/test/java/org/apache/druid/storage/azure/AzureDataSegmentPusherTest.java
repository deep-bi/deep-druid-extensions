...
public class AzureDataSegmentPusherTest extends EasyMockSupport {
    ...
    private static final String BLOB_PATH =
            " Россия 한국 中国!?/2015-04-12T00:00:00.000Z_2015-04-13T00:00:00.000Z/1/0/index.zip";
    ...
    private static final String NON_UNIQUE_NO_PREFIX_MATCHER =
            "foo/20150101T000000\\.000Z_20160101T000000\\.000Z/0/0/index\\.zip";
    private static final String NON_UNIQUE_WITH_PREFIX_MATCHER =
            PREFIX + "/" + "foo/20150101T000000\\.000Z_20160101T000000\\.000Z/0/0/index\\.zip";
    ...
    public void test_push_nonUniquePathNoPrefix_succeeds() throws Exception {
        ...
        AzureDataSegmentPusher pusher =
                new AzureDataSegmentPusher(azureStorage, azureAccountConfig, segmentConfigWithoutPrefix);
    ...
    public void test_push_nonUniquePathWithPrefix_succeeds() throws Exception {
        boolean useUniquePath = false;
        AzureDataSegmentPusher pusher =
                new AzureDataSegmentPusher(azureStorage, azureAccountConfig, segmentConfigWithPrefix);
        ...
        azureStorage.uploadBlob(
                EasyMock.anyObject(File.class), EasyMock.eq(CONTAINER_NAME), EasyMock.eq(PREFIX + "/" + azurePath));
        EasyMock.expectLastCall();
        ...
        Assert.assertTrue(
                segment.getLoadSpec().get("blobPath").toString(),
                Pattern.compile(NON_UNIQUE_WITH_PREFIX_MATCHER)
                        .matcher(segment.getLoadSpec().get("blobPath").toString())
                        .matches());
    ...
    public void test_push_uniquePath_succeeds() throws Exception {
        ...
        String azurePath = pusher.getAzurePath(SEGMENT_TO_PUSH, useUniquePath);
        azureStorage.uploadBlob(
                EasyMock.anyObject(File.class), EasyMock.eq(CONTAINER_NAME), EasyMock.matches(UNIQUE_MATCHER_PREFIX));
        EasyMock.expectLastCall();
    ...
    public void getAzurePathsTest() {
        ...
        Assert.assertEquals(
                StringUtils.format("%s/%s", storageDir, AzureStorageDruidModule.INDEX_ZIP_FILE_NAME), azurePath);
    }
