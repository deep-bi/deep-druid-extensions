...
public class AzureTestUtils extends EasyMockSupport {
    ...
    public static AzureCloudBlobIterable expectListObjects(
        ...
        EasyMock.expect(azureCloudBlobIterableFactory.create(ImmutableList.of(PREFIX_URI), maxListingLength))
    ...
    public static void expectDeleteObjects(
        ...
        for (Map.Entry<CloudBlobHolder, Exception> requestsAndErrors : deleteRequestToException.entrySet()) {
            ...
            IExpectationSetters<CloudBlobHolder> resultExpectationSetter =
                    requestToResultExpectationSetter.get(deleteObject);
            if (resultExpectationSetter == null) {
                storage.emptyCloudBlobDirectory(deleteObject.getContainerName(), deleteObject.getName());
                resultExpectationSetter =
                        EasyMock.<CloudBlobHolder>expectLastCall().andThrow(exception);
    ...
    public static CloudBlobHolder newCloudBlobHolder(String container, String prefix, long lastModifiedTimestamp)
            throws Exception {
        ...
        EasyMock.expect(object.getLastModifed())
                .andReturn(new Date(lastModifiedTimestamp))
                .anyTimes();
        return object;
    }
}
